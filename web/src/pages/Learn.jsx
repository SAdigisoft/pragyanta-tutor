...
export default function Learn() {
  ...
  useEffect(() => {
    ...
    Promise.all([getMessages(sessionId), getSession(sessionId)])
      .then(([history, metadata]) => {
        setMessages(history)
        setSession(metadata)
        setLevel(metadata.learner_level)
      })
  ...
  const ask = async (event, suggestion) => {
    ...
    setMessages((items) => [
      ...items,
      { id: `pending-${Date.now()}`, role: 'student', msg_type: 'chat', content: message },
    ])

    try {
      const result = await sendChat(sessionId, { message, learner_level: level })
      setMessages((items) => [...items.slice(0, -1), result.student_message, ...result.tutor_messages])
      setDemoState('custom')
    } catch (err) {
      setMessages((items) => items.slice(0, -1))
      setError(err.message || 'The tutor could not respond. Try asking again.')
    } finally {
      setThinking(false)
    }
  }
  ...
  const changeLevel = async (next) => {
    setLevel(next)
    await setLearnerLevel(sessionId, next)
  ...
  const switchState = async (state) => {
    ...
    if (state === 'error') {
      setMessages([])
      setError('The tutor could not respond. Your message was not sent.')
      return
    }
    await loadDemoState(state)
    setMessages(await getMessages(sessionId))
  }

  const lessonTitle = session?.lesson_title || 'Your lesson'
  const roleTargets = session?.lesson_id
    ? {
        teacher: `/report/${session.lesson_id}?role=teacher`,
        student: `/learn/${sessionId}?role=student`,
      }
    : {
  ...
  return (
    <div className="page learn-page">
      <Header mode="student" title={lessonTitle} roleTargets={roleTargets}>
        <LevelToggle value={level} onChange={changeLevel} />
      </Header>
      <main className="learn-main">
        <div className="lesson-rail">
          <Link to="/?role=teacher">← Leave lesson</Link>
        ...
        <section className="chat-shell" aria-label="Tutor conversation">
          {messages.length > 0 && (
            <div className="chat-heading">
              ...
              <p>Every answer is grounded in your teacher's material, not a live web or real-time knowledge API.</p>
            </div>
          )}
          <div className={`message-stream ${messages.length === 0 ? 'is-empty' : ''}`} aria-live="polite">
            {messages.length === 0 && !error && (
              <div className="chat-empty">
                ...
                <span className="lesson-prompt">Lesson prompt</span>
                <h2>
                  What would you like
                  <br />
                  to understand?
                </h2>
                <p>Ask about {lessonTitle}. I'll answer only from the uploaded lesson material, show the source, and refuse when the lesson does not cover a question.</p>
                <div className="starter-chips">
                  {starters.map((item, index) => (
                    <button key={item} onClick={() => ask(null, item)}>
            ...
            {messages.map((message, index) => (
              <ChatMessage message={message} key={message.id || `${message.role}-${message.msg_type}-${index}`} />
            ))}
            {thinking && (
              <div className="message-row tutor" role="status">
                <div className="typing">
                  <span />
                  <span />
                  <span />
                  <small>Finding evidence in your lesson</small>
                </div>
              </div>
            )}
            {error && (
              <div className="message-row tutor">
                <div className="chat-error" role="alert">
                  <div>
                    <strong>The tutor paused here</strong>
                    <p>{error}</p>
                  </div>
          ...
          <form className="composer" onSubmit={ask}>
            <div className="composer-row">
              ...
              <textarea
                ...
                placeholder={unavailable ? 'This session is unavailable' : 'Ask about this lesson...'}
                disabled={thinking || unavailable}
              />
              <button className="primary-button" disabled={!input.trim() || thinking || unavailable}>
                Ask the tutor <span>↗</span>
              </button>
            </div>
            <div className="composer-foot">
              <span>
                <i />
                Grounded in uploaded source sections
              </span>
